...
def webhook():
    ...
    # 將訊息存入全局變量
    global exchange_code, stock_code
    exchange_code = data['exchange']
    stock_code = data['symbol']
    event.set()
    print(data,exchange_code,stock_code)
...
# fo.bstrTriggerNumerator               # 觸發價分子。
    ...
    print("下單參數：", fo.__dict__)  # 除錯資訊
    # 海期下單 SendOverSeaFutureOrder(bstrLogInID, bAsyncOrder, pOrder)
    msg, nCode = skO.SendOverSeaFutureOrder(ID, 0, fo)
    print("下單結果：", msg, skC.SKCenterLib_GetReturnCodeMessage(nCode))  # 除錯資訊
    print(fo.bstrExchangeNo,fo.bstrStockNo)
    if nCode == 0:
        return "下單成功"
    else:
        return "下單失敗：" + skC.SKCenterLib_GetReturnCodeMessage(nCode)
